...
| **Run Code Analyzer**       | `sf code-analyzer run` scans **only the files changed in the PR** (diff vs merge-base, deletions excluded) with rule selector `SalesforceApexRules` (the custom PMD ruleset registered in `code-analyzer.yml`). PRs without `force-app` changes skip the scan entirely. Outputs JSON + HTML reports. **Severity threshold: 2** — any finding at severity 1 (Critical) or 2 (High) makes the run exit non-zero. |
